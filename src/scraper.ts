...
const debug = makeDebug('seonbi-core:scraper')
...
const getTrans = ($: Cheerio): string =>
  $.find('p.manyLang6')
    ...
    .replace(/[0-9]+\./, '')
    .trim()
const getKr = ($: Cheerio): string =>
  $.find('p.sub_p1')
    .first()
    .text()
const getEn = ($: Cheerio): string =>
  $.find('p.sub_p1.manyLang6')
    .first()
    .text()
